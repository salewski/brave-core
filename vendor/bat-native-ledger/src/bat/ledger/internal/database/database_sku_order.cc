...
void DatabaseSKUOrder::GetRecord(
  ...
  const std::string query = base::StringPrintf(
      "SELECT order_id, total_amount, merchant_id, status, created_at "
...
void DatabaseSKUOrder::OnGetRecord(
  ...
  info->status = static_cast<ledger::SKUOrderStatus>(GetIntColumn(record, 3));
  info->created_at = GetInt64Column(record, 4);
