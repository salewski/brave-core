...
void NTPBackgroundImagesService::StartSuperReferrerComponent(
  ...
  is_super_referrer_active_ = true;
...
void NTPBackgroundImagesService::StartSponsoredImagesComponent() {
  ...
  is_super_referrer_active_ = false;
  if (IsAlreadyRegistered(data->component_id))
    return;
...
void NTPBackgroundImagesService::OnComponentReady(
  ...
  // referral code.
  if (is_super_referrer != is_super_referrer_active_) {
...
bool NTPBackgroundImagesService::LoadTestData() {
  if (UseLocalTestData())
    return true;

  return UseBuiltInDemoData();
}
